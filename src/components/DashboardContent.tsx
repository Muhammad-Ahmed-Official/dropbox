...
import { FileText, FileUp, User, Wifi } from 'lucide-react'
import React, { useCallback, useEffect, useState } from 'react'
import FileUploadForm from './FileUploadForm'
import FileList from './FileList'
import UserProfile from './UserProfile'
import LocalShareTab from './LocalShare/LocalShareTab'
  ...
  return (
    <>
      ...
      <Tabs
        ...
        classNames={{ tabList: "gap-8", tab: "py-4 px-2", cursor: "bg-blue-600" }}
      >
        <Tab
          key="files"
          title={
        ...
        <Tab
          key="local-share"
          title={
            <div className={`flex items-center gap-3 ${activeTab === 'local-share' ? 'text-[#006fee]' : 'text-gray-500'}`}>
              <Wifi className="h-5 w-5" />
              <span className="font-semibold">Local Share</span>
            </div>
          }
        >
          <LocalShareTab />
