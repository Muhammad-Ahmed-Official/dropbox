...
class ApiClient {
  ...
  async createFolder(data: object) {
    return this.fetch("folders/create", { method: "POST", body: data });
  }

  async uploadFile(data: object) {
    return this.fetch("files/upload", { method: "POST", body: data });
  }
  ...
  async starFiles(fileId: string) {
    return this.fetch(`files/${fileId}/star`, { method: "PATCH" });
  }

  async delFiles(fileId: string) {
    return this.fetch(`files/${fileId}/delete`, { method: "DELETE" });
  }

  async trashFile(fileId: string) {
    return this.fetch(`files/${fileId}/trash`, { method: "PATCH" });
  }

  async emptyTrash() {
    return this.fetch(`files/empty-trash`, { method: "DELETE" });
  }
}
